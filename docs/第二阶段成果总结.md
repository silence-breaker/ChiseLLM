...
- `data_gen/generator.py` 切换至安全的 `sys.stdout.reconfigure` 调用路径，兼容缺失该方法的 Python 运行时。
- 进程池提示信息更加友好，JVM 预热阶段的“停滞”行为已得到明确说明。
- 实测样本：`conda activate chisel-llm && python data_gen/generator.py 3 1`，3/3 样本全部通过编译与阐述验证。

## 3. 数据资产

- `dataset/chisel_sft_dataset.jsonl` 现已收录最新的高质量样本。
