# reflect_env.py 使用指南 (v2.2)

## 简介

`reflect_env.py` 是 ChiseLLM 项目的核心模块，提供了自动化测试 Chisel 代码的完整功能。这是本项目的**唯一编译和验证方式**。

> ⚠️ **重要**: 本项目不使用 VS Code Metals 等 IDE 集成，所有编译验证通过 `run_reflect.py` 完成。
...
## v2.2 更新日志 (2025-11-25)

### 重大变更
- 🔄 使用 Mill 构建工具替代 sbt (编译速度更快)
- 🔒 版本锁定: Scala 2.13.12 + Chisel 6.0.0 (与数据集保持一致)
- ❌ 不再支持 VS Code Metals IDE 集成

### 新功能
- ✨ 支持静默模式 (`silent=True`) 用于批量数据生成
- ✨ Mill 1.x API 语法支持

### 改进
- ⚡ 编译速度显著提升 (Mill vs sbt)
- 📝 更新文档以反映新的工作流程
